...
H = 3  # как в build_merged/train_nhits_dualmove.py


# === индикаторы (идентичны build_merged.py) ===
...
def _get_klines(symbol: str, interval: str, limit: int = 600) -> pd.DataFrame:
    ...
    return df[["time", "open", "high", "low", "close", "volume"]].drop_duplicates("time").sort_values("time")


def build_live_frame(sym: str) -> pd.DataFrame:
    m15 = _get_klines(sym, Client.KLINE_INTERVAL_15MINUTE)
    h1 = _get_klines(sym, Client.KLINE_INTERVAL_1HOUR)
    h4 = _get_klines(sym, Client.KLINE_INTERVAL_4HOUR)

    if m15.empty or h1.empty or h4.empty:
        raise RuntimeError(f"[{sym}] no klines")
...
def get_usdt_balance() -> float | None:
    try:
        ...
        return float(x["balance"]) if x else None
    except Exception:
        return None


def _sleep_to_next_close():
    server_time = client.get_server_time()["serverTime"] // 1000
    next_close = ((server_time // 900) + 1) * 900
    sleep_for = max(0, next_close - server_time - 3)
    if sleep_for > 0:
        print(f"Wait {sleep_for:.1f}s to next 15m close")
        time.sleep(sleep_for)


def main():
    nf_up_map, nf_down_map = load_models()

    while True:
        _sleep_to_next_close()
        ...
        for sym in SYMBOLS:
            try:
                df = build_live_frame(sym)
                data_by_symbol[sym] = df
            except Exception as e:
                print(f"[{sym}] build error: {e}")

        if not data_by_symbol:
            print("No data, skip cycle")
            continue

        preds = get_predictions(nf_up_map, nf_down_map, data_by_symbol)
        if preds.empty:
            print("No predictions, skip cycle")
            continue
        ...
        for _, r in preds.iterrows():
            sym = r["symbol"]
            up = float(r["up_pred"])
            down = float(r["down_pred"])
            price = float(client.futures_mark_price(symbol=sym)["markPrice"])
            print(f"{sym:7} up={up:.6f} down={down:.6f} price={price:.4f}")
