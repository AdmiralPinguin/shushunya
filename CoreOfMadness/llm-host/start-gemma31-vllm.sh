#!/bin/bash
# Gemma-4-31B QAT-W4A16 on vLLM, port 8080 — the smarter head.
# SINGLE-3090 profile (tight: 22GB weights on a 24GB card): ctx 6144, eager mode,
# one sequence, 1024-token prefill chunks, and no CUDA graphs. When the second
# 3090 arrives, switch to TP2: TENSOR_PARALLEL=2, MAX_MODEL_LEN=32768, and drop
# --enforce-eager (see comments below).
# The old gguf model name is served as an alias so the dispatcher (8079) and every
# client keep working with zero changes. 12B fallback: start-gemma-vllm.sh.
cd /media/shushunya/SHUSHUNYA/shushunya || exit 1

RTX3090_UUID="GPU-a5dffcde-00f4-8625-fd65-193cfd964696"   # pinned by UUID, not index
LOG=CoreOfMadness/vllm-host/runtime/gemma31-vllm-8080.log
PID_FILE=CoreOfMadness/vllm-host/runtime/gemma31-vllm-8080.pid
MODEL=CoreOfMadness/models/google-gemma-4-31B-it-qat-w4a16-ct
mkdir -p CoreOfMadness/vllm-host/runtime

# any other gemma must not hold the port or the VRAM
if pgrep -f "llama-server.*gemma-4-12b" >/dev/null 2>&1; then
  echo "stopping llama.cpp gemma…"; pkill -f "llama-server.*gemma-4-12b"; sleep 3
fi
if pgrep -f "vllm serve.*gemma-4-12B-it-qat" >/dev/null 2>&1; then
  echo "stopping vLLM gemma-12B…"; pkill -f "vllm serve.*gemma-4-12B-it-qat"; sleep 5
fi

if curl -fsS --max-time 3 http://127.0.0.1:8080/v1/models >/dev/null 2>&1; then
  echo "gemma (8080) already up"; exit 0
fi

# inductor needs nvcc; the toolkit lives inside the venv (pip nvidia-cu13), and the
# drive likes to drop exec bits — restore them or triton's ptxas/nvcc fail with EACCES.
CUDA_HOME_DIR="$PWD/CoreOfMadness/vllm-host/venv/lib/python3.12/site-packages/nvidia/cu13"
find CoreOfMadness/vllm-host/venv -path "*/bin/*" -type f ! -perm -u+x -exec chmod +x {} + 2>/dev/null

TP="${TENSOR_PARALLEL:-1}"
if [ "$TP" -gt 1 ]; then
  # TP2 (second 3090): graphs on, vision back, real concurrency and context.
  TIGHT_FLAGS=""
  DEFAULT_CTX=32768
  DEFAULT_GPU_MEMORY_UTILIZATION=0.98
else
  # single 3090: 22GB weights on a 24GB card — no CUDA graphs, no vision tower,
  # one sequence and 1024-token prefill chunks. The 0.94/6144 envelope leaves
  # activation headroom that the old 0.98/7936 profile did not have.
  TIGHT_FLAGS="--enforce-eager --language-model-only --max-num-seqs 1 --max-num-batched-tokens 1024"
  DEFAULT_CTX=6144
  DEFAULT_GPU_MEMORY_UTILIZATION=0.94
  export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
fi

: > "$LOG"
# flashinfer refuses to JIT on the torch-cu130/nvcc-13.2 mix — disable it.
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES="${GEMMA_GPUS:-$RTX3090_UUID}" \
CUDA_HOME="$CUDA_HOME_DIR" \
PATH="$PWD/CoreOfMadness/vllm-host/venv/bin:$CUDA_HOME_DIR/bin:$PATH" \
VLLM_USE_FLASHINFER_SAMPLER=0 VLLM_ATTENTION_BACKEND=FLASH_ATTN \
setsid nohup CoreOfMadness/vllm-host/venv/bin/vllm serve "$MODEL" \
  --served-model-name gemma-4-12b-it-UD-Q5_K_XL.gguf google/gemma-4-31B-it-qat-w4a16-ct \
  --host 127.0.0.1 --port 8080 \
  --tensor-parallel-size "$TP" \
  --max-model-len "${MAX_MODEL_LEN:-$DEFAULT_CTX}" \
  --gpu-memory-utilization "${GPU_MEMORY_UTILIZATION:-$DEFAULT_GPU_MEMORY_UTILIZATION}" \
  --dtype auto --trust-remote-code $TIGHT_FLAGS ${EXTRA_ARGS:-} \
  >> "$LOG" 2>&1 </dev/null &
echo "$!" > "$PID_FILE"
echo "gemma-31B vLLM pid $(cat "$PID_FILE"), log $LOG"
