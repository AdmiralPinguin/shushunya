...
class CeraxiaLifecycleTests(unittest.TestCase):
    ...
    def test_real_explicit_patch_pipeline_reaches_execution_ready(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(worker_report["code_worker_pipeline"]["final_manifest"]["status"], "ready")
            self.assertEqual(worker_report["code_worker_pipeline"]["verification_report"]["status"], "passed")
            self.assertTrue(worker_report["code_worker_pipeline"]["repair_loop_state"])
            verification = json.loads((run_dir / "verification_report.json").read_text(encoding="utf-8"))
            self.assertEqual(verification["status"], "passed")
            self.assertTrue(verification["commands_executed"])
    ...
    def test_project_creation_mode_builds_greenfield_project(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp) / "new_project"
            repo.mkdir()
            spec = {
                "summary": "demo app",
                "files": [
            ...
            result = run_ceraxia(
                CeraxiaInput(
                    task="Создай новый минимальный python проект `demo`.\nCERAXIA_PROJECT:\n" + json.dumps(spec),
                    repo_path=str(repo),
    ...
    def test_review_gate_marks_failed_surface_verification(self) -> None:
        ...
        self.assertTrue(repair_item["attempt_history_policy"]["required"])
        self.assertEqual(repair_item["attempt_history_policy"]["max_attempts"], 3)
        self.assertTrue(repair_item["attempt_history_policy"]["dedupe_key"])
        self.assertTrue(repair_item["attempt_history_policy"]["block_repeat_without_new_evidence"])
        self.assertIn("result", repair_item["attempt_history_policy"]["required_fields"])
        self.assertIn("same dedupe_key fails after 2 attempts", repair_item["replan_required_when"])
        self.assertEqual(repair_item["escalation_policy"]["escalate_to"], "PlanningBrigade")
        self.assertEqual(repair_item["escalation_policy"]["after_attempts"], 3)
        self.assertTrue(repair_item["escalation_policy"]["requires_new_hypothesis_before_retry"])
