...
gates, handoff, and read-only boundary. `role_service.py` exposes those same
...
roles plus the handoff contracts above. The role services expose the same
outputs one role at a time; Ceraxia still owns final packet approval and
handoff.
