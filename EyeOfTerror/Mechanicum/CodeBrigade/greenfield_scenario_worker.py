...
def _feature_scenarios(feature: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    table: dict[str, list[dict[str, Any]]] = {
        ...
        "maintenance_work_orders_api": [
            _scenario("maintenance_domain_workflow", "Maintenance service creates work orders, assigns technicians, and transitions statuses", ["create work order", "assign technician", "transition status"], ["create_work_order", "assign_technician", "transition_status", "resolved"], ["app/domain.py", "tests/test_maintenance.py"]),
            _scenario("maintenance_store_summary", "Maintenance service filters work orders and summarizes status counts", ["filter by technician", "filter by status", "summarize by status"], ["WorkOrderStore", "technician", "summary_by_status", "in_progress"], ["app/store.py", "tests/test_maintenance.py"]),
            _scenario("maintenance_route_adapters", "Maintenance service exposes route adapter helpers and FastAPI router wiring", ["create route response", "assign route response", "transition route response", "summary route response", "include router"], ["create_order_response", "assign_order_response", "transition_order_response", "summary_response", "include_router"], ["app/routes.py", "app/main.py", "tests/test_maintenance.py"]),
        ],
        "inventory_ops_api": [
            _scenario("inventory_crud_ledger", "Inventory API creates, updates, and adjusts stock with a ledger", ["create inventory item", "update item metadata", "adjust stock", "record ledger row"], ["create_item", "update_item", "adjust_stock", "StockAdjustment", "ledger"], ["app/domain.py", "app/store.py", "tests/test_inventory_ops.py"]),
            _scenario("inventory_low_stock_filters", "Inventory API reports low-stock items and filters inventory by SKU/category/status/search", ["filter inventory", "compute low-stock report", "serialize item results"], ["list_inventory_response", "low_stock_report", "category", "status", "search"], ["app/store.py", "app/reports.py", "app/routes.py", "tests/test_inventory_ops.py"]),
            _scenario("inventory_route_errors", "Inventory API exposes route adapters and JSON error payloads", ["create route response", "adjust stock route response", "return JSON error objects"], ["create_item_response", "adjust_stock_response", "error_response", "json_error", "include_router"], ["app/routes.py", "app/main.py", "tests/test_inventory_ops.py"]),
        ],
        "operations_dashboard_api": [
            _scenario("operations_service_incident_lifecycle", "Operations dashboard registers services and tracks incident lifecycle", ["register service", "record incident", "resolve incident"], ["register_service", "record_incident", "resolve_incident", "resolved"], ["app/domain.py", "app/store.py", "tests/test_operations_dashboard.py"]),
            _scenario("operations_metrics_workflow", "Operations dashboard computes health metrics from services and incidents", ["compute service health", "count open incidents", "build dashboard metrics"], ["service_health_score", "build_dashboard_metrics", "open_incident_count", "health_by_service"], ["app/metrics.py", "tests/test_operations_dashboard.py"]),
            _scenario("operations_event_timeline", "Operations dashboard builds service and incident event timeline", ["build service event", "build incident event", "order event timeline"], ["service_event", "incident_event", "build_event_timeline", "incident_resolved"], ["app/events.py", "tests/test_operations_dashboard.py"]),
            _scenario("operations_route_adapters", "Operations dashboard exposes route adapter helpers and FastAPI router wiring", ["register route response", "record incident route response", "dashboard route response", "include router"], ["register_service_response", "record_incident_response", "dashboard_response", "include_router"], ["app/routes.py", "app/main.py", "tests/test_operations_dashboard.py"]),
        ],
        "csv_summary": [
...
def _template_scenarios(project_type: str, template_id: str) -> list[dict[str, Any]]:
    table = {
        ...
        "static_browser_game": [_scenario("browser_game_loop", "Browser game loads a canvas, handles input, renders frames, and tracks score", ["open HTML", "load game script", "handle keyboard controls", "run animation loop", "update score"], ["canvas", "requestAnimationFrame", "Arrow", "score"], ["index.html", "game.js", "tests/test_browser_game.py"])],
