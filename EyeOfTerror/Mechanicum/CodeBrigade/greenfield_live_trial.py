...
SCENARIO_TASKS: dict[str, dict[str, Any]] = {
    "python_cli": {
        "task": DEFAULT_TASK,
        "expected_template_id": "python_cli_basic",
        "expected_features": [],
    },
    "operations_dashboard": {
        "task": (
            "Создай FastAPI backend service `ops-dash-demo`: operations dashboard для сервисов. "
            "Нужны регистрация сервисов, incident tracking, resolve incident, health metrics, event timeline, "
            "HTTP route adapters, разделение domain/store/metrics/events/routes/main и тесты без live server."
        ),
        "expected_template_id": "python_fastapi_service",
        "expected_features": ["operations_dashboard_api"],
    },
    "sales_pipeline": {
        "task": (
            "Создай data sales analytics pipeline `sales-pipeline-demo` для CSV: загрузка записей, фильтрация, "
            "группировка выручки по регионам, markdown report, CLI JSON output и контрактные тесты."
        ),
        "expected_template_id": "data_processing_tool",
        "expected_features": ["sales_analytics_pipeline"],
    },
    "local_agent_router": {
        "task": (
            "Создай local agent tool router `agent-router-demo`: registry команд status/echo/summarize, "
            "валидация JSON payload, session history, sequence runner, отказ неизвестных action и CLI."
        ),
...
def compact_greenfield_result(result: dict[str, Any], workspace: Path) -> dict[str, Any]:
    ...
    project_brief = greenfield_project.get("greenfield_project_brief") if isinstance(greenfield_project.get("greenfield_project_brief"), dict) else {}
    ...
    acceptance_features = [
        str(row.get("id") or "")
        for row in project_brief.get("acceptance_features", [])
    ...
    return {
        ...
        "run_dir": str(run_dir),
        "project_type": str(project_brief.get("project_type") or ""),
        "template_id": str(project_brief.get("template_id") or ""),
        "expected_files_count": len(project_brief.get("expected_files", [])) if isinstance(project_brief.get("expected_files"), list) else 0,
        "module_contract_count": len(project_brief.get("module_contracts", [])) if isinstance(project_brief.get("module_contracts"), list) else 0,
        "scenario_count": int((project_brief.get("scenario_plan") or {}).get("scenario_count") or 0) if isinstance(project_brief.get("scenario_plan"), dict) else 0,
...
def scenario_spec(name: str) -> dict[str, Any]:
    try:
        return SCENARIO_TASKS[name]
    except KeyError as exc:
        raise ValueError(f"unsupported greenfield live scenario: {name}") from exc


def apply_scenario_expectations(result: dict[str, Any], scenario: str, spec: dict[str, Any]) -> dict[str, Any]:
    expected_template = str(spec.get("expected_template_id") or "")
    expected_features = [str(item) for item in spec.get("expected_features", []) if isinstance(item, str)]
    missing_features = [feature for feature in expected_features if feature not in result.get("acceptance_features", [])]
    expectation_blockers: list[str] = []
    if expected_template and result.get("template_id") != expected_template:
        expectation_blockers.append(f"expected template_id {expected_template}, got {result.get('template_id')}")
    if missing_features:
        expectation_blockers.append("missing expected acceptance features: " + ", ".join(missing_features))
    result["scenario"] = scenario
    result["expected_template_id"] = expected_template
    result["expected_features"] = expected_features
    result["scenario_expectation_status"] = "passed" if not expectation_blockers else "blocked"
    result["scenario_expectation_blockers"] = expectation_blockers
    if expectation_blockers:
...
def run_live_scenario(scenario: str, run_root: Path) -> dict[str, Any]:
    spec = scenario_spec(scenario)
    result = run_live_trial(str(spec["task"]), run_root)
    result = apply_scenario_expectations(result, scenario, spec)
    workspace = Path(str(result.get("workspace") or ""))
    if workspace.name == "workspace":
        trial_root = workspace.parent
        (trial_root / "live_greenfield_trial_result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return result


def main() -> int:
    parser = argparse.ArgumentParser(description="Run a live-model Ceraxia greenfield project trial without replay guidance.")
    parser.add_argument("--scenario", choices=sorted(SCENARIO_TASKS), help="Named no-replay greenfield stress scenario.")
    ...
    result = run_live_scenario(args.scenario, args.run_root) if args.scenario else run_live_trial(args.task, args.run_root)
