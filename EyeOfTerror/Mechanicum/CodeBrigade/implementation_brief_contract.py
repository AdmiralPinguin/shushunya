...
def validate_implementation_brief(brief: dict[str, Any]) -> list[str]:
    ...
    if execution_intent.get("mode") not in {"planning_handoff_only", "explicit_patch_execution", "guarded_inferred_patch_execution", "greenfield_project_creation"}:
        problems.append("brief execution_intent.mode is required")
    if execution_intent.get("adapter_capability") not in {"explicit_patch_adapter_only", "explicit_or_guarded_inference_adapter", "greenfield_project_scaffold_adapter"}:
