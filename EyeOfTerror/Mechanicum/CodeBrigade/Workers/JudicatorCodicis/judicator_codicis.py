...
def run(request: dict[str, Any], workspace_root: Path) -> dict[str, Any]:
    ...
    return run_code_review(request, workspace_root, codewright_core.output_path_from_request(request))


def main() -> int:
    parser = argparse.ArgumentParser(description="Run JudicatorCodicis code review worker.")
