...
def run_trial(trial_id: str, timeout_sec: int) -> dict[str, Any]:
    env = {**os.environ, "PYTHONPATH": str(ROOT)}
    completed = subprocess.run(
        [sys.executable, str(RUNNER), "--trial", trial_id],
        cwd=str(ROOT.parent),
        env=env,
        text=True,
        capture_output=True,
...
def main() -> int:
    ...
    results = [run_trial(trial_id, args.timeout_sec) for trial_id in expert_trial_ids()]
