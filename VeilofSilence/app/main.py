...
def _mic_thread():
    ...
    try:
        # выясняем, сколько входных каналов доступно (Pulse/pipewire отдаёт 1 или 2)
        info = sd.query_devices(dev, 'input') if dev is not None else sd.query_devices(None, 'input')
        ch = info.get('max_input_channels', 1) or 1
        ch = 1 if ch < 1 else min(ch, 2)
        if DEBUG_VEIL: print(f"[Veil][MIC] channels={ch}")
        ...
        block = int(TARGET_SR * 0.02)  # 20ms
        with sd.RawInputStream(samplerate=TARGET_SR, blocksize=block, dtype='int16',
                               channels=ch, callback=cb, device=dev):
            buf = b''
            need = TARGET_SR * 2 * 3 // 2  # ~1.5s PCM16
            while True:
                buf += q.get()
                if len(buf) >= need:
                    chunk, buf = buf[:need], buf[need:]
                    # упаковываем в WAV и шлём в собственный /stt
                    b = io.BytesIO()
                    arr = np.frombuffer(chunk, dtype=np.int16)
                    sf.write(b, arr, TARGET_SR, format='WAV', subtype='PCM_16')
                    b.seek(0)
                    try:
                        r = httpx.post(f"http://127.0.0.1:{getenv('SERVER_PORT',8011)}/stt",
                                       files={"file":("mic.wav", b.getvalue(), "audio/wav")},
                                       data={"lang":"ru"}, timeout=30)
                        if DEBUG_VEIL: print("[Veil][MIC]->/stt", r.status_code)
                    except Exception as e:
                        print("[Veil][MIC] send error:", repr(e))
    except Exception as e:
        print("[Veil][MIC] init error:", repr(e))
        traceback.print_exc()
