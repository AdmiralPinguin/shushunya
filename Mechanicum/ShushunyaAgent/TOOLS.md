...
link-local, multicast, reserved, unspecified, and credential-bearing URL
targets. `web_fetch` returns status, final URL, content type, title, extracted
text, and truncation status.
Binary responses return `is_binary=true` and byte metadata without decoded text.
Unknown text encodings fall back to UTF-8 instead of failing the tool call.
